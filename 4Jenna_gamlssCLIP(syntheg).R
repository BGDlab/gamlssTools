...
gven <- ggplot(data = analysisDf, aes(x = age_in_years,y = Ventricles, col = fs_version))+
  geom_point()+
  labs(title = "Ventricles vs Age")+  
  theme(legend.position = "none")

gwmv <- ggplot(data = analysisDf, aes(x = age_in_years,y = WMV, col = fs_version))+
  geom_point()+
  labs(title = "WMV vs Age")+
  theme(legend.position = "none")

ggmv <- ggplot(data = analysisDf, aes(x = age_in_years,y = GMV, col = fs_version))+
  geom_point()+
  labs(title = "GMV vs Age")+
  theme(legend.position = "none")

gsgmv <- ggplot(data = analysisDf, aes(x = age_in_years,y = sGMV, col = fs_version))+
  geom_point()+
...
centiles.fan(TCV2, xvar=analysisDf$age_in_years, cent=c(3,10,25,50,75,90,97), colors = "topo",
             ylab="TCV", xlab = "age(years)")


Ven1 <- gamlss(Ventricles~pb(age_in_years), sigma.formula=~pb(age_in_years),nu.formula=~pb(age_in_years), 
               data = analysisDf, family = GG)
Ven2 <- gamlss(Ventricles~pb(age_in_years), sigma.formula=~pb(age_in_years), nu.formula=~pb(age_in_years),
...
centiles.fan(Ven2, xvar=analysisDf$age_in_years, cent=c(3,10,25,50,75,90,97), colors = "topo",
             ylab="Ventricles", xlab = "age(years)")


Ceb1 <- gamlss(WMV~pb(age_in_years), sigma.formula=~pb(age_in_years),nu.formula=~pb(age_in_years), 
               data = analysisDf, family = GG)
Ceb2 <- gamlss(WMV~pb(age_in_years), sigma.formula=~pb(age_in_years), nu.formula=~pb(age_in_years),
...
Gray1 <- gamlss(sGMV~pb(age_in_years), sigma.formula=~pb(age_in_years),nu.formula=~pb(age_in_years), 
                data = analysisDf, family = GG)
Gray2 <- gamlss(sGMV~pb(age_in_years), sigma.formula=~pb(age_in_years), nu.formula=~pb(age_in_years),
