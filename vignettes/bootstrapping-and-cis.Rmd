...
```{r}
#get true peak
sim_df <- sim_grid(df, "Age", "Sex")
cent <- centile_fan_values(pheno_model, sim_df, "Age", centiles = c(0.5), average_over = TRUE)
...
#get true peak
sim_df <- sim_grid(df, "Age", "Sex")
cent_by_sex <- centile_fan_values(pheno_model, sim_df, "Age", centiles = c(0.5))
