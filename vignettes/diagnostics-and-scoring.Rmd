...
centile_coverage(centiles=test_df$centiles2)
```

`centile_coverage()` takes `batch_term` too, and hands it straight to `score_centiles()`, so you can
skip the intermediate step when you only want the coverage:
