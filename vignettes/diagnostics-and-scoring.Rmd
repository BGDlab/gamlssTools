...
```{r}
test_control$centiles <- score_centiles(pheno_model, test_control)
...
```{r, eval = has_charts}
#get centiles 
test_control$centiles2 <- score_centiles(pheno_model, test_control, batch_term="Study")
hist(test_control$centiles2)
...
```{r, eval = has_charts}
test_new$centiles <- score_centiles(pheno_model, test_new, batch_term="Study", ref_data= ~ dx=="control")

ggplot(test_new) +
  geom_histogram(aes(x=centiles, fill=dx), position="dodge", alpha=.6) +
  theme_minimal()
```
