...
#'
#' @export
sim_grid <- function(data, x_var, factor_var=NULL, gamlssModel=NULL, special_term=NULL, x_range = NULL){
...
remove_effects.gamlss <- function(gamlssModel, data, ref_data=NULL, rm_terms=NULL, zero_terms=NULL){
  ...
  for (col in rm_terms){
    if (is.numeric(ref_data[[col]])){
      new_df[[col]] <- mean(ref_data[[col]], na.rm = TRUE)
    } else if (is.factor(ref_data[[col]])) {
      mode_value <- mode(ref_data[[col]])
      new_df[[col]] <- factor(rep(mode_value, nrow(new_df)), levels = levels(ref_data[[col]]))
    } else {
      new_df[[col]] <- mode(ref_data[[col]])
    }
  }
...
remove_effects.gamlss2 <- function(gamlssModel, data, ref_data=NULL, rm_terms=NULL, zero_terms=NULL){
  ...
  required_cols <- c(rm_terms, zero_terms, as.character(get_y(gamlssModel)))
  missing_cols_df <- setdiff(required_cols, names(df))
  missing_cols_og <- setdiff(required_cols, names(ref_data))

  if (length(missing_cols_df) > 0) {
    stop(paste("Missing columns in df:", paste(missing_cols_df, collapse = ", ")))
  }
  if (length(missing_cols_og) > 0) {
    stop(paste("Missing columns in ref_data:", paste(missing_cols_og, collapse = ", ")))
  }

  # Ensure data types match between df and ref_data for rm_terms
  for (col in rm_terms) {
    if (class(df[[col]]) != class(ref_data[[col]])) {
      warning(paste("Data type mismatch for column", col, "- converting df[[col]] to match ref_data[[col]]"))
      if (is.factor(ref_data[[col]])) {
        df[[col]] <- factor(df[[col]], levels = levels(ref_data[[col]]))
...
#' and `rm_terms` arguments map onto `data`, `ref_data` and `rm_terms` respectively.
#' @export
